    ...
    try:
        if url:
            ...
            response = requests.get(
                url, auth=(USERNAME, PASSWORD), headers=headers, timeout=300
            )
        else:
...
def save_to_database(detections, update_existing=False):
    ...
    try:
        ...
        for detection in tqdm(detections, desc="Saving to database"):
            if update_existing:
                ...
                if existing:
                    ...
                    existing.last_scan_datetime = parse_datetime(
                        detection.get("LastScanDateTime")
                    )
                    existing.vulnerability_title = detection.get(
                        "VulnerabilityTitle", ""
                    )  # Added title
                    ...
                    existing.first_found_datetime = parse_datetime(
                        detection.get("FirstFoundDateTime")
                    )
                    existing.last_found_datetime = parse_datetime(
                        detection.get("LastFoundDateTime")
                    )
                    existing.last_test_datetime = parse_datetime(
                        detection.get("LastTestDateTime")
                    )
                    existing.last_update_datetime = parse_datetime(
        ...
        if update_existing:
            print(
                f"✅ Database updated: {new_records} new records, {updated_records} updated records"
            )
        else:
...
def export_to_csv(detections, filename=None):
    ...
    try:
        ...
        column_mapping = {
            ...
            "QID": "QID",
            "VulnerabilityTitle": "Vulnerability Title",  # Added title
...
def show_summary(detections):
    ...
    print(f"Vulnerability Titles Fetched: {len(qid_title_cache)}")
...
def main():
    ...
    if choice == "1":
        ...
        if detections:
            show_summary(detections)
            new_records, updated_records = save_to_database(
                detections, update_existing=True
            )
        else:
            print("❌ No data fetched")
                    ...
                    detections.append(
                        {
                            ...
                            "LastScanDateTime": (
                                record.last_scan_datetime.isoformat()
                                if record.last_scan_datetime
                                else ""
                            ),
                            "UniqueVulnID": record.unique_vuln_id,
                            "QID": record.qid,
                            "VulnerabilityTitle": record.vulnerability_title
                            or "",  # Added title
                            ...
                            "FirstFoundDateTime": (
                                record.first_found_datetime.isoformat()
                                if record.first_found_datetime
                                else ""
                            ),
                            "LastFoundDateTime": (
                                record.last_found_datetime.isoformat()
                                if record.last_found_datetime
                                else ""
                            ),
                            "LastTestDateTime": (
                                record.last_test_datetime.isoformat()
                                if record.last_test_datetime
                                else ""
                            ),
                            "LastUpdateDateTime": (
                                record.last_update_datetime.isoformat()
                                if record.last_update_datetime
                                else ""
                            ),
        ...
        try:
            ...
            critical_count = (
                db.query(QualysData)
                .filter(QualysData.qds_severity == "CRITICAL")
                .count()
            )
            high_count = (
                db.query(QualysData).filter(QualysData.qds_severity == "HIGH").count()
            )
            unique_assets = db.query(QualysData.asset_id).distinct().count()
            records_with_titles = (
                db.query(QualysData)
                .filter(QualysData.vulnerability_title != "")
                .count()
            )
            ...
            print(f"Records with Vulnerability Titles: {records_with_titles}")
        ...
        xml_data, status_code = fetch_qualys_detection_page(
            params={"action": "list", "output_format": "XML", "truncation_limit": 1}
        )
        ...
        if xml_data:
            ...
            if detections:
                print(f"Sample titles fetched: {len(qid_title_cache)}")
        else:
